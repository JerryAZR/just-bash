...
- **File writes**: up to ~5 MB total per execution. The run sync bridge serializes the stack frame for each sync host call, and cumulative args across all calls stall above ~6 MB. Reads are unaffected (return values are not in the frame). For larger writes, write from the shell side (not js-exec) or use multiple exec calls.
